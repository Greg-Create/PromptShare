...
function cardPage() {
    ...
    useEffect(()=>{
        ...
        supabase.from("posts").select().eq("id",postId).then(result=> {
            if(result.error){
                throw result.error
            }
            if(result.data){
      ...
      useEffect(()=>{
        ...
        supabase.from("profiles").select().eq("id",authorIds).then(result=> {
            if(result.error){
                throw result.error
            }
            if(result.data){
            ...
            
            {
              ...
              <Comments id={postId}/>:
                 <div className="locked">
                    ...
                    <p><Link href="/SignIn">Login</Link> to comment</p> 
                     </div>
            }
        </div>
        </div>
